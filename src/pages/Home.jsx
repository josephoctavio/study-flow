...
import { BookOpen, Clock, Zap, MapPin, ClipboardList, Layout, ChevronDown, ChevronUp, AlertCircle, WifiOff, PlayCircle } from 'lucide-react';
...
const Home = ({ theme, darkMode, userName, stats, todayClasses, loading }) => {
  ...
  // Restored Dynamic Colors
  const hasTasks = stats?.totalTasks > 0;
  const isFullyComplete = hasTasks && stats.completedTasks === stats.totalTasks;
  const currentColor = !hasTasks ? colors.accent : 
                   isFullyComplete ? '#34C759' : 
                   stats.percentage < 35 ? '#FF3B30' : 
                   stats.percentage < 75 ? '#FFCC00' : '#34C759';
    ...
    return (
      <div style={{ padding: '20px', backgroundColor: colors.bg, minHeight: '100vh' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div style={{ width: '140px', height: '24px', backgroundColor: colors.card, borderRadius: '8px' }} className="shimmer" />
          <div style={{ width: '80px', height: '28px', backgroundColor: colors.card, borderRadius: '12px' }} className="shimmer" />
        </div>
        <div style={{ height: '160px', backgroundColor: colors.card, borderRadius: '28px', border: `1px solid ${colors.border}` }} className="shimmer" />
        <style>{`.shimmer { animation: pulse 1.5s infinite ease-in-out; } @keyframes pulse { 0% { opacity: 0.5; } 50% { opacity: 0.8; } 100% { opacity: 0.5; } }`}</style>
      </div>
    );
  }
  ...
  // Class is upcoming if it ends in the future
  const upcomingClasses = allSorted.filter(c => timeToMinutes(c.end_time) > currentTotalMinutes);
  const pastClasses = allSorted.filter(c => timeToMinutes(c.end_time) <= currentTotalMinutes);

  const nextClass = upcomingClasses.find(c => timeToMinutes(c.start_time) > currentTotalMinutes);

  const renderClassCard = (item, isPast = false) => {
    const isLive = !isPast && currentTotalMinutes >= timeToMinutes(item.start_time) && currentTotalMinutes < timeToMinutes(item.end_time);
    ...
    return (
      <div key={item.id} style={{ 
        ...
        borderRadius: '22px', 
        border: `1px solid ${isLive ? colors.accent : colors.border}`, 
        display: 'flex', 
        alignItems: 'center', 
        gap: '14px',
        ...
        {isLive && <div style={{ position: 'absolute', top: 0, left: 0, width: '4px', height: '100%', backgroundColor: colors.accent }} />}
        
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
             <div style={{ fontSize: '15px', fontWeight: '800', color: colors.text, textDecoration: isPast ? 'line-through' : 'none' }}>
               {item.courses?.name}
             </div>
             {isLive && (
               <span style={{ fontSize: '8px', fontWeight: '900', backgroundColor: colors.accent, color: '#fff', padding: '2px 6px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '3px', animation: 'pulse 2s infinite' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '6px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#666', fontWeight: '700' }}>
              <Clock size={12} color={isPast ? '#444' : (isLive ? colors.accent : '#888')} /> 
  ...
  return (
    <div style={{ padding: '20px', paddingBottom: '120px', color: colors.text, backgroundColor: colors.bg, position: 'relative', minHeight: '100vh' }}>
      ...
      <header style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', backgroundColor: status.bg, borderRadius: '12px' }}>
      ...
      {/* 2. PROGRESS CARD (Restored Colors) */}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '10px', fontWeight: '900', color: '#666' }}>DAILY PROGRESS</span>
            </div>
            <h2 style={{ fontSize: '28px', fontWeight: '900', margin: 0 }}>{stats?.percentage || 0}%</h2>
          </div>
          <div style={{ textAlign: 'right' }}>
            <span style={{ fontSize: '14px', fontWeight: '900', color: currentColor }}>{stats?.completedTasks || 0}/{stats?.totalTasks || 0}</span>
            <p style={{ margin: 0, fontSize: '9px', fontWeight: '800', opacity: 0.4 }}>TASKS</p>
          </div>
        </div>
        <div style={{ width: '100%', height: '8px', backgroundColor: darkMode ? '#000' : '#F0F0F0', borderRadius: '10px', marginTop: '20px', overflow: 'hidden' }}>
          <div style={{ width: `${stats?.percentage || 0}%`, height: '100%', backgroundColor: currentColor, transition: 'width 1s cubic-bezier(0.34, 1.56, 0.64, 1)' }} />
        </div>
      </div>
      ...
      <div style={{ marginBottom: '12px', padding: '0 4px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <h3 style={{ fontSize: '11px', fontWeight: '900', color: colors.text, letterSpacing: '1px', margin: 0 }}>SCHEDULE</h3>
          <span style={{ fontSize: '10px', fontWeight: '800', color: colors.accent }}>{fullDateStr}</span>
        </div>
        {nextClass && <div style={{ fontSize: '10px', fontWeight: '800', color: colors.accent, marginTop: '4px' }}>Next: {nextClass.courses?.name} in {(() => {
      ...
      {/* 5. PAST CLASSES (Restored Hide Toggle) */}
